...
function normalizeUser(user) {
  ...
  return {
    ...
    // Role comes from the user_roles table (see js/auth.js -> buildUser).
    // The client caches it here for UI gating; RLS on the backend is
    // what actually enforces admin-only writes.
...
// Sync helper — returns cached user immediately (used by ensureAccess).
function restoreSession() {
  return getCurrentUser();
}

// Async — refreshes cached user from Supabase (call at page load).
async function refreshSessionFromCloud() {
  const sb = window.sbClient;
  const { data } = await sb.auth.getUser();
  if (!data?.user) { clearCurrentUser(); return null; }
  const built = await StudyCoreAuth.buildUser(data.user);
  if (!built) return null;
  return persistUser(built);
}
...
// --- Cloud-backed auth (Supabase) ---
async function registerUser({ name, email, password, learningLevel = 'secondary' }) {
  const sb = window.sbClient;
  const { data, error } = await sb.auth.signUp({
    email: email.trim().toLowerCase(),
    password,
    options: { data: { name: name.trim(), learning_level: parseLearningLevel(learningLevel) } }
  });
  if (error) return { ok: false, message: error.message };
  // Poll briefly for the trigger-created profile+role, then build the user.
  let built = null;
  for (let i = 0; i < 6 && !built?.role; i++) {
    await new Promise(r => setTimeout(r, 250));
    built = await StudyCoreAuth.buildUser(data.user);
  }
  if (!built) return { ok: false, message: 'Account created — please log in.' };
  persistUser(built);
  setRememberedLogin(built.email);
  return { ok: true, user: built };
}
...
async function loginUser(email, password, remember = true) {
  ...
  if (error) return { ok: false, message: error.message };
  const built = await StudyCoreAuth.buildUser(data.user);
  if (!built) return { ok: false, message: 'Could not load your profile.' };
  if (built.isActive === false) {
    await sb.auth.signOut();
    return { ok: false, message: 'Your account has been deactivated.' };
  }
  persistUser(built);
  if (remember) setRememberedLogin(built.email);
...
async function updateUserProfile(updates) {
  ...
  if (updates.learningLevel != null) dbUpdates.learning_level = parseLearningLevel(updates.learningLevel);
  if (updates.school != null) dbUpdates.school = updates.school;
  if (updates.grade != null) dbUpdates.grade = updates.grade;
  if (updates.subscription != null) dbUpdates.subscription = updates.subscription;
  if (updates.darkMode != null) dbUpdates.dark_mode = !!updates.darkMode;
  if (Object.keys(dbUpdates).length) {
    await sb.from('profiles').update(dbUpdates).eq('id', currentUser.id);
  }
...
async function logoutUser() {
  try { await window.sbClient.auth.signOut(); } catch {}
  clearCurrentUser();
...
function renderAssignments(containerId = 'assignmentList', subjectFilter = '') {
  const container = document.getElementById(containerId);
  if (!container) return;
  const assignments = getStoredList('studycore_assignments', DEFAULT_ASSIGNMENTS)
    .filter((item) => !subjectFilter || matchesResourceSubject(item.subject, subjectFilter));
  if (!assignments.length) {
    container.innerHTML = '<div class="info-card"><h3>No assignments available yet</h3><p>Check back soon for new assignments.</p></div>';
    return;
  }
...
function renderQuizzes(containerId = 'quizList', subjectFilter = '') {
  const container = document.getElementById(containerId);
  if (!container) return;
  const quizzes = getStoredList('studycore_quizzes', DEFAULT_QUIZZES)
    .filter((item) => !subjectFilter || matchesResourceSubject(item.subject, subjectFilter));
  if (!quizzes.length) {
    container.innerHTML = '<div class="info-card"><h3>No quizzes available yet</h3><p>Check back soon for new quizzes.</p></div>';
    return;
  }
  ...
  container.querySelectorAll('[data-quiz-index]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      runQuiz(selected, containerId);
    });
  });
}

function runQuiz(quiz, containerId = 'quizList') {
  if (!quiz) return;
  const container = document.getElementById(containerId);
  if (!container) return;
  let score = 0;
...
function renderAssignmentsPage() {
  renderAssignments('assignmentCards', getUrlSubject());
}

function renderQuizzesPage() {
  renderQuizzes('quizCards', getUrlSubject());
}
...
function handleAuthForms() {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', async function (event) {
      event.preventDefault();
      console.log('[StudyCore] signup submit fired');
      const submitBtn = signupForm.querySelector('button[type="submit"]');
      if (submitBtn) { submitBtn.disabled = true; submitBtn.dataset.origLabel = submitBtn.textContent; submitBtn.textContent = 'Creating account…'; }
      const name = document.getElementById('signupName')?.value || '';
      const email = document.getElementById('signupEmail')?.value || '';
      const password = document.getElementById('signupPassword')?.value || '';
      const learningLevel = document.getElementById('signupLevel')?.value || 'secondary';
      const result = await registerUser({ name, email, password, learningLevel });
      if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = submitBtn.dataset.origLabel || 'Create Free Account'; }
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function (event) {
      ...
      if (submitBtn) { submitBtn.disabled = true; submitBtn.dataset.origLabel = submitBtn.textContent; submitBtn.textContent = 'Signing in…'; }
      const email = document.getElementById('loginEmail')?.value || '';
      const password = document.getElementById('loginPassword')?.value || '';
      const remember = document.getElementById('rememberMe')?.checked || true;
      const result = await loginUser(email, password, remember);
      if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = submitBtn.dataset.origLabel || 'Log In'; }
      if (!result.ok) { showToast(result.message, 'error'); return; }
...
function isPastPaperItem(item) {
  ...
  return text.includes('past paper') || text.includes('exam paper') || text.includes('revision pack');
}
...
function renderResourceCards(type, containerId, options = {}) {
  const container = document.getElementById(containerId);
  if (!container) return;
  ...
  if (!items.length) {
    container.innerHTML = `<div class="info-card"><h3>No ${options.label || type} available yet</h3><p>Check back soon — new ${options.label || type}s uploaded by StudyCore admins will appear here.</p></div>`;
    return;
  }
...
function initResourcePages() {
  ...
  if (window.location.pathname.includes('/documents.html')) {
    renderResourceCards('document', 'documentCards', {
      label: 'document',
  ...
  if (window.location.pathname.includes('/videos.html')) {
    renderResourceCards('video', 'videoCards', {
      label: 'video',
      subject: getUrlSubject()
    });
  }
...
async function hydrateContentFromCloud() {
  const sb = window.sbClient;
  try {
    const [videos, documents, quizzes, assignments, announcements] = await Promise.all([
      sb.from('videos').select('*').eq('publish_status', 'published').order('created_at', { ascending: false }),
      sb.from('documents').select('*').eq('publish_status', 'published').order('created_at', { ascending: false }),
      sb.from('quizzes').select('*').eq('publish_status', 'published').order('created_at', { ascending: false }),
      sb.from('assignments').select('*').eq('publish_status', 'published').order('created_at', { ascending: false }),
      sb.from('announcements').select('*').eq('publish_status', 'published').order('created_at', { ascending: false }),
    ]);
    if (videos.data) saveStoredList('studycore_video_resources', videos.data.map(v => ({ id: v.id, title: v.title, subject: v.subject, description: v.description, link: v.url })));
    if (documents.data) saveStoredList('studycore_document_resources', documents.data.map(d => ({ id: d.id, title: d.title, subject: d.subject, description: d.description, link: d.url })));
    if (quizzes.data && quizzes.data.length) saveStoredList('studycore_quizzes', quizzes.data.map(q => ({ id: q.id, title: q.title, subject: q.subject, questions: q.questions })));
    if (assignments.data && assignments.data.length) saveStoredList('studycore_assignments', assignments.data.map(a => ({ id: a.id, title: a.title, subject: a.subject, description: a.description, due: a.due_date })));
    if (announcements.data && announcements.data.length) saveStoredList('studycore_announcements', announcements.data.map(a => ({ id: a.id, title: a.title, detail: a.detail, createdAt: a.created_at })));
  } catch (e) { console.warn('[StudyCore] content hydrate failed', e); }
}

window.addEventListener('DOMContentLoaded', async () => {
  try { await refreshSessionFromCloud(); } catch (e) { console.warn(e); }
  await hydrateContentFromCloud();
...
// Auth forms are handled in handleAuthForms() (see initAuthSystem).
